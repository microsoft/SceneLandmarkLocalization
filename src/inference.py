...
def inference(opt, minimal_tight_thr=1e-2, opt_tight_thr=5e-3, mode='test'):
    ...
    test_dataloader = DataLoader(dataset=test_dataset, num_workers=1, batch_size=8, shuffle=False, pin_memory=True)
    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(tqdm(test_dataloader)):
            ...
            if use_gt_2d3d:
                xy1 = copy.deepcopy(landmark2d)
                xy1[:, :2] += 5.0 * np.random.randn(B, 2, num_landmarks)                
                C_B_f = K_inv.cpu().numpy() @ xy1
                C_B_f /= np.linalg.norm(C_B_f, axis=1, keepdims=True)
                xy1[:, :2] /= opt.output_downsample
                P = batch['visibility'].float().cpu().numpy()
            else:
    ...
    ## Number of 2D-3D detections for poses that are within 5cm 5 deg
    _2d3d_lessthan5cm = {'count': [], 'median': [], 'mean': []}
    _2d3d_morethan5cm = {'count': [], 'median': [], 'mean': []}
    _2d3d_morethan10cm = {'count': [], 'median': [], 'mean': []}
    for imgdata in test_image_logging:
        if imgdata['trans_err_all'] < 0.05:
            _2d3d_lessthan5cm['count'].append(len(imgdata['pixel_error']))
            _2d3d_lessthan5cm['median'].append(np.median(imgdata['pixel_error']))
            _2d3d_lessthan5cm['mean'].append(np.mean(imgdata['pixel_error']))
        else:
            _2d3d_morethan5cm['count'].append(len(imgdata['pixel_error']))
            _2d3d_morethan5cm['median'].append(np.median(imgdata['pixel_error']))
            _2d3d_morethan5cm['mean'].append(np.mean(imgdata['pixel_error']))
        
        if imgdata['trans_err_all'] > 0.1:
            _2d3d_morethan10cm['count'].append(len(imgdata['pixel_error']))
            _2d3d_morethan10cm['median'].append(np.median(imgdata['pixel_error']))
            _2d3d_morethan10cm['mean'].append(np.mean(imgdata['pixel_error']))

    print('---- images whose estimated poses error are less than 5cm ----')    
    print('count [min/max/median]: ', np.min(_2d3d_lessthan5cm['count']), '/', np.max(_2d3d_lessthan5cm['count']), '/', np.median(_2d3d_lessthan5cm['count']))
    print('median [min/max/median]: ', np.min(_2d3d_lessthan5cm['median']), '/', np.max(_2d3d_lessthan5cm['median']), '/', np.median(_2d3d_lessthan5cm['median']))

    print('---- images whose estimated poses error are more than 5cm ----')
    print('count [min/max/median]: ', np.min(_2d3d_morethan5cm['count']), '/', np.max(_2d3d_morethan5cm['count']), '/', np.median(_2d3d_morethan5cm['count']))
    print('median [min/max/median]: ', np.min(_2d3d_morethan5cm['median']), '/', np.max(_2d3d_morethan5cm['median']), '/', np.median(_2d3d_morethan5cm['median']))

    print('---- images whose estimated poses error are more than 10cm ----')
    print('count [min/max/median]: ', np.min(_2d3d_morethan10cm['count']), '/', np.max(_2d3d_morethan10cm['count']), '/', np.median(_2d3d_morethan10cm['count']))
    print('median [min/max/median]: ', np.min(_2d3d_morethan10cm['median']), '/', np.max(_2d3d_morethan10cm['median']), '/', np.median(_2d3d_morethan10cm['median']))
